...
public class JiraTest5 {
	...
	public static void main(String[] args) {
		...
		String response=given().relaxedHTTPSValidation().header("Content-type", "application/json").body("{ \"username\": \"nirupa\", \r\n"
		...
		for(int i=0;i<commentscount;i++) {
			...
			if(commentid.equalsIgnoreCase(actualcommentid)) {
				comment=js1.get("fields.comment.comments["+i+"].body").toString();
